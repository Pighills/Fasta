...
2. **Publicera** uppgifter med status `godkänd`, en i taget: uppdatera grenen mot `main`, höj cache-versionen i `sw.js` till nästa lediga nummer, kör `node --check` och `node --test tests/*.test.mjs`, gör ett kort test i 390×844 och slå ihop enligt git-flödet i AGENTS.md. Sätt status `live` med cache-version i kön och uppdatera raden "Live nu" i `docs/STATUS.md`. Publicera aldrig något som inte har status `godkänd`.
